...
//   - contaminate A/B/A navigation (B data never shown in A; A shows
//     only A's own server truth).
...
for (const outcome of ["success", "failure"] as const) {
  test(`A -> B -> A: a delayed ${outcome} of A's first save never contaminates B nor shows B data back in A`, async () => {
    ...
    try {
      ...
      const truthA = (globalThis as any).__server[A_ID].find((m: any) => m.modeCode === "pickup").customerText;
      assert.equal(truthA, outcome === "success" ? "RETRAIT-A-NOUVEAU" : "RETRAIT-A-INITIAL");
      assert.equal(textareaValue(container, "pickup"), truthA, "A shows only A's own server truth");
      assert.equal(textareaValue(container, "delivery"), "LIVRAISON-A-INITIAL");
      assert.equal(readLog().includes(B_ID), true, "B was really loaded during the round-trip");
      assert.deepEqual(mutationLog(), [`${A_ID}:pickup:RETRAIT-A-NOUVEAU`], "only A was written, never B");
