...
RENDER_HELPER="$SCRIPT_DIR/seller-legal-profile-cgv-engine-v2-2-render-helper.mjs"
...
# et il s'agit bien de la ligne version=1 (is_default=true), MÊME
# APRÈS l'installation de v2.1 (version 2, jamais is_default) ET de
# v2.2 (version 3, jamais is_default non plus) -- la preuve
# comportementale centrale de ce cycle, réaffirmée ici pour que TOUTES
# les assertions qui comparent leur propre résolution à TEMPLATE_ID
# plus bas dans ce fichier vérifient, de fait, cette même invariance.
...
assert_eq "0bis-note (v2.2): le gabarit RÉELLEMENT applicable (TEMPLATE_ID, is_default=true) == la ligne version=1 -- v2.2 a restauré cette invariance que v2.1 avait rompue (versions 2 et 3 existent, sont PUBLISHED, mais ne sont JAMAIS is_default)" "$TEMPLATE_ID_V1_ONLY" "$TEMPLATE_ID"
...
# Restauration : is_default revient sur la version 1, et le gabarit
# simulé (version 99) est retiré -- pour que le gabarit applicable FR
# redevienne EXACTEMENT TEMPLATE_ID (la vraie version 1) pour le reste
# du harnais (dont [ROLLBACK]).
RESTORE_RC=$(psql -d "$DB" -v ON_ERROR_STOP=1 -c "
  update public.cgv_template set is_default = false where id = '$FR_TEMPLATE_VNEXT_ID';
  update public.cgv_template set is_default = true where id = '$TEMPLATE_ID_V1_ONLY';
  delete from public.cgv_template where id = '$FR_TEMPLATE_VNEXT_ID';
" >/dev/null 2>&1; echo $?)
assert_eq "CONCURRENCY 4/12+5/12: restauration (is_default=false sur la ligne v99 AVANT is_default=true sur la ligne v1, même ordre que l'activation -- respecte l'index unique partiel) réussit" "0" "$RESTORE_RC"
...
assert_eq "CONCURRENCY 4/12+5/12: après restauration, le gabarit is_default redevient EXACTEMENT TEMPLATE_ID_V1_ONLY (aucune corruption résiduelle pour le reste du harnais)" "$TEMPLATE_ID_V1_ONLY" "$RESTORE_CHECK"
...
# RESTO_A (retour au défaut is_default, version 1) AVANT la résolution
# de republication ci-dessous -- pour que la ligne merchant_cgv_version
# ACTIVE laissée par ce bloc, et TOUTE résolution ultérieure de RESTO_A
# dans le reste du harnais, redevienne EXACTEMENT TEMPLATE_ID (version
# 1), l'invariance centrale que le reste de ce fichier suppose partout.
as_authenticated_rc "$OWNER_A" \
  "select public.update_merchant_cgv_profile('$RESTO_A','EXEMPT_PERISHABLE',20,30,'MINUTES','Nouvelle politique','Nouvelle substitution','FORMAL',false,'FIXED_PORTION_PRICE');" >/dev/null
...
resolve_for_persist_rc "$OWNER_A" "$RESTO_A" >/dev/null
resolve_split
assert_eq "V21-FIXED-PORTION-PRICE/V21-COLD-CHAIN cleanup: après désépinglage, RESTO_A résout à nouveau EXACTEMENT TEMPLATE_ID (version 1, is_default)" "$TEMPLATE_ID" "$RESOLVED_TID"
...
assert_eq "V22-TEMPLATE-ACTIVATION -- NON-RÉGRESSION CRITIQUE: RESTO_A (non épinglé) résout la version 1, PAS la version 3, bien que la version 3 existe et soit PUBLISHED" "1" "$RESTO_A_RESOLVED_VERSION"
...
assert_eq "V22-TEMPLATE-ACTIVATION -- NON-RÉGRESSION CRITIQUE (chemin RÉEL de publication): resolve_cgv_publication_context renvoie template_version=1 pour RESTO_A, jamais 3" "1" "$(sql_out)"
...
assert_eq "V22-TEMPLATE-ACTIVATION: get_applicable_cgv_template renvoie ÉGALEMENT version=1 pour RESTO_A -- l'aperçu et la publication réelle restent en accord après ce lot (plomberie nécessaire, voir l'en-tête de la migration)" "1" "$(sql_out)"
...
# (jamais épinglé, résout TOUJOURS is_default = version 1) et on
# l'ÉPINGLE EXPLICITEMENT sur CE MÊME id (version 1) entre resolve et
# persist : le gabarit RÉSOLU reste identique (v_applicable.id ==
# p_template_id, la ré-preuve d'autorité passe), mais
# pinned_template_id passe de NULL à non-NULL -- le SEUL changement
# possible, et donc la preuve que le fingerprint (et lui seul) détecte
# ce changement.
VERSION_COUNT_BEFORE=$(sql "select count(*) from public.merchant_cgv_version where restaurant_id='$RESTO_A';")
resolve_for_persist_rc "$OWNER_A" "$RESTO_A" >/dev/null
resolve_split
assert_eq "V22-STALE-CONTEXT-PIN setup: le gabarit résolu pour RESTO_A (non épinglé) est bien la version 1 (is_default)" "$V1_TEMPLATE_ID" "$RESOLVED_TID"
psql -d "$DB" -v ON_ERROR_STOP=1 -c "update public.merchant_cgv_profile set pinned_template_id = '$V1_TEMPLATE_ID' where restaurant_id = '$RESTO_A';" >/dev/null
...
# ------------------------------------------------------------------
# v2.2 -- Au Lait Cru est désormais EXPLICITEMENT ÉPINGLÉE sur le
# gabarit version 3 (mandat v2.2, item 5 : "explicitly selected/
# activated for Au Lait Cru"). AUCUN changement à is_default -- la
# version 1 reste is_default=true pour TOUT le monde d'autre, et cette
# écriture directe sur merchant_cgv_profile.pinned_template_id est
# l'unique mécanisme utilisé, jamais public.persist_merchant_cgv_
# version, jamais une modification de cgv_template.is_default.
# ------------------------------------------------------------------
ALC_V3_ID=$(sql "select id from public.cgv_template where template_code='FR_FOOD_PERISHABLE_B2C' and version=3;")
psql -d "$DB" -v ON_ERROR_STOP=1 -c "update public.merchant_cgv_profile set pinned_template_id = '$ALC_V3_ID' where restaurant_id = '$RESTO_ALC';" >/dev/null
pass "AU-LAIT-CRU: pinned_template_id pointe explicitement sur FR_FOOD_PERISHABLE_B2C version 3 -- AUCUN changement à is_default (toujours la version 1 pour tout autre marchand)"
ALC_RESOLVED_VERSION=$(sql "select version from public._resolve_applicable_cgv_template('$RESTO_ALC');")
assert_eq "AU-LAIT-CRU: _resolve_applicable_cgv_template résout bien la version 3 pour Au Lait Cru (épinglée)" "3" "$ALC_RESOLVED_VERSION"
OTHER_MERCHANT_STILL_V1=$(sql "select version from public._resolve_applicable_cgv_template('$RESTO_A');")
assert_eq "AU-LAIT-CRU: épingler Au Lait Cru sur la version 3 n'a AUCUN effet sur un autre marchand (RESTO_A résout toujours la version 1)" "1" "$OTHER_MERCHANT_STILL_V1"
