...
# SELLER LEGAL PROFILE + CGV ENGINE + ACCEPTANCE SNAPSHOT v1.3 —
# SCOPE-REVIEW-APPROVED NARROW REMEDIATION, OPTION A (FULL) — closes
# the three residual gaps of CGV-V11-PUBLISH-CONTEXT-RACE-01 that
# remained OPEN after the Cycle 3 verdict on v1.2. PostgreSQL réel,
# aucune simulation applicative, exécuté en tant qu'utilisateur système
# postgres (authentification peer).
#
# Ce fichier est un COPIE/ADAPTATION directe de
# seller-legal-profile-cgv-engine-v1-2-check.sh : TOUTES les sections
# v1.2 (installation de la chaîne, [BLOCKER2/ACL-0], fixtures, [0bis],
# [BLOCKER1/RPC-BYPASS], [BLOCKER1/GRANTS], [1]..[27], [BLOCKER1/
# TEMPLATE-AUTHORITY], [BLOCKER2/ACL], la matrice [BLOCKER1/CONCURRENCY]
# à 12 items, et la démonstration [BLOCKER1/LOCKING] existante sur
# merchant_legal_profile) sont conservées SANS MODIFICATION DE FOND —
# seuls les noms de variables/DB et l'installation d'un lot
# supplémentaire (v1.3, par-dessus v1.1+v1.2) ont changé — afin de
# prouver que v1.3 ne régresse RIEN de ce que v1.2 avait déjà fermé.
#
# BASELINE : main —
...
# PUIS v1.2 (tous deux déjà audités, blockers CLOSED, non rouverts ici)
# PUIS v1.3 (ce cycle) par-dessus, exactement comme les trois fichiers
# sont chaînés en Production (v1.1 forward, puis les deltas v1.2 et
# v1.3 forward).
#
# v1.3 ferme trois gaps résiduels nommés par la revue de périmètre
# CTO-approuvée, SANS changer aucune signature de fonction ni aucun
# grant (deux CREATE OR REPLACE seulement) :
#
#   GAP 1 — le fingerprint de contexte (_compute_cgv_publication_
#   context_fingerprint) ne hashait pas restaurants.name (le
#   seller_name imprimé tel quel dans le document rendu), et se fiait à
#   template.id+version comme PROXY du contenu du gabarit plutôt que de
#   hasher directement cgv_template.controlled_sections::text. Testé
#   ci-dessous par [V13-GAP1/SELLER-NAME] et
#   [V13-GAP1/CONTROLLED-SECTIONS].
#
#   GAP 2 — persist_merchant_cgv_version ne verrouillait pas la ligne
#   restaurants ni la ligne cgv_template avant sa comparaison finale de
#   fingerprint et sa mutation. Testé ci-dessous par
#   [V13-GAP2/LOCKING-RESTAURANTS] (démonstration de verrouillage réel,
#   même patron que [BLOCKER1/LOCKING] mais sur la ligne restaurants).
#
#   GAP 3 — la re-vérification d'autorisation (_assert_legal_cgv_role_
#   for_user) s'exécutait comme une lecture non verrouillée, avant tout
#   verrou sur la ligne autorisante elle-même, laissant une fenêtre où
#   une révocation concurrente pouvait s'intercaler sans jamais être
#   observée. Testé ci-dessous par [V13-GAP3/LOCKING-AUTH-ORDER1] et
#   [V13-GAP3/LOCKING-AUTH-ORDER2] (les deux ordres d'acquisition du
#   verrou, démonstration de verrouillage réel PostgreSQL, pas une
#   assertion de code source).
#
# Usage : depuis la racine du dépôt (contenant supabase/) :
#   sudo -n -u postgres bash supabase/tests/seller-legal-profile-cgv-engine-v1-3-check.sh
# ============================================================
...
# v1.3 (ce cycle) s'installe PAR-DESSUS v1.1+v1.2, exactement comme en
# Production (delta forward-only, deux CREATE OR REPLACE sans
# changement de signature -- vérifié par le pre-flight ET le
# post-verify du fichier v1.3 lui-même).
...
CODE_ONLY=$(grep -hv '^\s*--' "$LOT_SQL_V11" "$LOT_SQL_V12" "$LOT_SQL_V13")
assert_not_contains "10. le SQL exécuté (hors commentaires, v1.1+v1.2+v1.3) ne référence jamais delay_value/delay_unit" "delay_value" "$(printf '%s' "$CODE_ONLY" | grep -o 'delay_value\|delay_unit' || true)"
...
CODE_ONLY_SCOPE=$(grep -hv '^\s*--' "$LOT_SQL_V11" "$LOT_SQL_V12" "$LOT_SQL_V13")
assert_not_contains "25. le SQL exécuté (v1.1+v1.2+v1.3) ne touche aucune table de statut de paiement" "payment_status" "$(printf '%s' "$CODE_ONLY_SCOPE" | grep -oiE 'payment_status|monetico' || true)"
assert_not_contains "26. le SQL exécuté (v1.1+v1.2+v1.3) ne fait aucun appel Stuart (une référence à un autre fichier de provenance, en commentaire d'en-tête, est exclue de cette recherche)" "stuart" "$(printf '%s' "$CODE_ONLY_SCOPE" | grep -oi 'stuart' || true)"
