#!/usr/bin/env bash
# ============================================================
# Scanym — INVOICE REQUEST PRODUCTION PREREQUISITE REMEDIATION v1.2
# — TARGETED ROLLBACK FINGERPRINT COMPLETENESS FIX ONLY — Claude Monet
#
...
# guards. Supersedes the v1.1 10-case matrix -- keeps all 10 original
# cases (function drift, SECURITY DEFINER/search_path drift, column
# type/default drift, added constraint/index, RLS state drift, policy
# predicate/role drift, grant drift) and adds 6 more required by
# Catimini's v1.1 re-audit (grant to an unexpected role, TRUNCATE
# grant, REFERENCES grant, TRIGGER grant, policy permissive-to-
# restrictive change, column collation change). Sixteen scenarios,
# each:
...
DB="scanym_invreq_negmatrix_v12_$$"
...
# ------------------------------------------------------------
log "=== [1/16] Fonction set_order_invoice_request : même signature, CORPS altéré ==="
...
# ------------------------------------------------------------
log "=== [2/16] Fonction set_order_invoice_request : SECURITY DEFINER retiré (devient INVOKER) ==="
...
# ------------------------------------------------------------
log "=== [3/16] Colonne vat_number : type TEXT -> VARCHAR(30) ==="
...
# ------------------------------------------------------------
log "=== [4/16] Colonne created_at : DEFAULT now() -> clock_timestamp() ==="
...
# ------------------------------------------------------------
log "=== [5/16] Contrainte CHECK inattendue ajoutée ==="
...
# ------------------------------------------------------------
log "=== [6/16] Index inattendu ajouté sur country ==="
...
# ------------------------------------------------------------
log "=== [7/16] RLS : FORCE ROW LEVEL SECURITY activé (état non attendu) ==="
...
# ------------------------------------------------------------
log "=== [8/16] Politique RLS : prédicat USING élargi (drift de sécurité) ==="
...
# ------------------------------------------------------------
log "=== [9/16] Politique RLS : rôle inattendu ajouté (service_role) ==="
...
# ------------------------------------------------------------
log "=== [10/16] GRANT inattendu : anon obtient SELECT (drift de sécurité critique) ==="
...
# ------------------------------------------------------------
log "=== [11/16] GRANT SELECT à un rôle INATTENDU (service_role, drift de sécurité) ==="
...
# ------------------------------------------------------------
log "=== [12/16] GRANT TRUNCATE inattendu ==="
...
# ------------------------------------------------------------
log "=== [13/16] GRANT REFERENCES inattendu ==="
...
# ------------------------------------------------------------
log "=== [14/16] GRANT TRIGGER inattendu ==="
...
# ------------------------------------------------------------
log "=== [15/16] Politique RLS : PERMISSIVE -> RESTRICTIVE (même prédicat, même rôle) ==="
...
# specifically named)
# ------------------------------------------------------------
log "=== [16/16] Colonne contact_name : COLLATION par défaut -> \"C\" ==="
