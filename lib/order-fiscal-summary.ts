...
/**
 * TVA / HT / TTC COMPLETION v1 -- CONTRAT DE CALCUL FISCAL UNIQUE.
 *
 * Ce module est l'AUTORITÉ fiscale unique d'une commande. Il ne rend
 * rien : il retourne des nombres. Trois consommateurs partagent donc
 * exactement les mêmes chiffres, sans jamais redupliquer la logique
 * (mandat §4) :
 *
 *     instantané fiscal de la commande
 *              |
 *              +--> affichage back-office (components/dashboard/OrderCard.tsx)
 *              +--> ticket imprimé        (lib/receipt.ts)
 *              +--> facture future        (lot Invoice -- consommer computeOrderFiscalSummary)
 *
 * ORIGINE DU CODE -- ce fichier n'invente AUCUN calcul. Les fonctions
 * `roundCents`, `buildMixedRateTaxGroups`, `hasCompleteDeliveryTaxSnapshot`
 * et toute la logique de décision ci-dessous sont DÉPLACÉES TELLES
 * QUELLES depuis lib/receipt.ts (STUART LOT C v1.2->v1.5 + MERCHANT
 * LEGAL & TAX PROFILE v1.1), où elles étaient privées au moteur de
 * rendu HTML et donc inaccessibles au back-office et à la facture.
 * Les conventions d'arrondi existantes sont PRÉSERVÉES À L'IDENTIQUE
 * (mandat §5 : ne pas inventer de nouvelle stratégie d'arrondi) -- la
 * preuve est un test "golden" qui compare le HTML du ticket produit
 * après extraction à celui produit avant, octet pour octet.
 *
 * INVARIANT COMPTABLE (mandat §3) : toutes les entrées proviennent
 * EXCLUSIVEMENT des instantanés persistés au moment de la commande
 * (`order.tax_settings_snapshot_*`, `order_items.tax_rate_snapshot`,
 * `order.order_delivery_tax_allocations`, `order.total`/`subtotal`).
 * JAMAIS le catalogue courant, jamais le taux marchand courant : une
 * commande historique reste donc fiscalement stable même après un
 * changement de prix ou de taux.
 */
...
/** Une ligne du détail TVA : un taux, sa base HT, sa TVA, son TTC. */
export interface FiscalRateRow {
  /** Taux en pourcentage (ex. 5.5, 10, 20). */
  rate: number;
  /** Base hors taxes pour ce taux. */
  net: number;
  /** Montant de TVA pour ce taux. */
  tax: number;
  /** Montant TTC pour ce taux (net + tax). */
  gross: number;
}
 ...
 *   (c'est le seul mode qui peut porter plusieurs taux).
 * - `flat-rate`  : instantané marchand à taux unique -> HT/TVA/TTC.
...
export type OrderFiscalSummary =
  ...
  | {
      ...
      taxLabel: string;
      totalGross: number;
      /** Pourquoi aucune décomposition n'est exposée (traçabilité). */
      reason:
        | "no-tax-snapshot"
 ...
 *
 * Fonction PURE : aucune E/S, aucune dépendance au rendu. C'est le
 * point d'entrée unique du mandat §4 -- back-office, ticket et future
 * facture doivent tous l'appeler plutôt que recalculer.
 */
export function computeOrderFiscalSummary(
  order: DashboardOrder,
  options?: {
    /**
     * Libellé de taxe de repli, utilisé UNIQUEMENT quand la commande ne
     * porte aucun instantané fiscal. C'est de l'AFFICHAGE, jamais un
     * montant : le comportement historique de lib/receipt.ts (repli sur
     * `receipt_settings.tax_label`) est ainsi préservé à l'identique,
     * tout en gardant cette fonction pure et sans dépendance aux
     * réglages marchands.
     */
    fallbackTaxLabel?: string | null;
  }
): OrderFiscalSummary {
  ...
  if (useMixedRateRendering) {
    ...
    return {
      ...
      totalNet: mixedRateTotalNet,
      // Somme des TVA par taux -- jamais un recalcul depuis le total :
      // chaque composante conserve sa propre frontière d'arrondi
      // (convention LOT C v1.3, inchangée).
  ...
  if (showTax && !suppressVatBreakdownForIncompleteHistory) {
    return {
      mode: "flat-rate",
      taxLabel: taxLabelResolved,
      rate,
      totalNet: excludingTax,
      totalTax: taxAmount,
